...
The trailers other tools and projects write count the same way: Cursor's `Made-with: Cursor`, Claude Code's `Claude-Session:` link, the Apache Software Foundation's `Generated-by: <tool>`, and QEMU's `AI-used-for: code|tests|docs`, whose value is kept as the scope. Code generators and people write `Made-with` and `Generated-by` too, so those two count only when they name a known AI tool (Claude, GitHub Copilot, Cursor, Codex, Gemini, ChatGPT, Codeium, Tabnine); `Generated-by: protoc-gen-go` is not AI attribution.
